...
PixelBasedAdaptiveSegmenter::PixelBasedAdaptiveSegmenter() : firstTime(true), showOutput(false), enableInputBlur(true), enableOutputBlur(true),
...
void PixelBasedAdaptiveSegmenter::process(const cv::Mat &img_input, cv::Mat &img_output, cv::Mat &img_bgmodel)
{
  ...
  pbas.process(&img_input_new, &img_foreground, &img_bgmodel);
